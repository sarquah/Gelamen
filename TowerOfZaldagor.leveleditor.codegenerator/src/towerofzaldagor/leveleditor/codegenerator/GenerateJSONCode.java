...
public class GenerateJSONCode implements IObjectActionDelegate 
{
	...
	public void run(IAction action) {
		...
		final String relativeUri = "templates/TOZModel.jsonjet";
		final JETEmitter emitter = new JETEmitter(base + relativeUri, getClass().getClassLoader());
		String className = act.getActName();		
		final String fileName = className + ".json";
		IContainer[] containers = WorkspaceResourceDialog.openFolderSelection(null, "Save JSON code...",
				"Please select a folder to which the generated JSON code should go.", false, new IContainer[] { container }, null);
		if (containers.length == 1) {
			container = containers[0];
			final IFile file = container.getFile(new Path(fileName));
			final Job job = new Job("Generating JSON code for the TOZ model: ") {
				@Override
				protected IStatus run(IProgressMonitor monitor) {
					try {				
						emitter.addVariable("LEVELEDITOR2JSON", Activator.PLUGIN_ID);
